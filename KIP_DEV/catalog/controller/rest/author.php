 ...
 */
class ControllerRestAuthor extends Controller {
	...
	*/
  	public function author() {
		...
		if(isset($this->request->get['id'])){
			$author = $this->model_catalog_information->getInformation($this->request->get['id']);
			if($author != null){
				$json['success']	= true;
				$json["data"]['author'][] = array(
					'id'  		 => $author['information_id'],
					'title'      => $author['title'],
					'description' => str_replace("\n"," ",str_replace("\r","",strip_tags(html_entity_decode($author['description'], ENT_QUOTES, 'UTF-8'))))
				);
			}else{
				$json['error']	= "Author is not available.";
			}
		}else{
				$json['error']	= "Please give id in URL.";
		}
		if ($this->debugIt) {
			echo '<pre>';
			print_r($json);
			...
			//echo htmlentities (print_r (json_encode ($json), true));
		}
	}
